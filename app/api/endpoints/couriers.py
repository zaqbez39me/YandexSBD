...
from app.api.dependencies.couriers import (create_courier, get_courier, get_couriers_in_range,
                                           get_courier_orders_in_time_interval, get_courier_metadata,
                                           date_to_datetime_start, date_to_datetime_end, get_couriers_assignments)
from app.schemas.models.common import int32
from app.schemas.models.orders import OrderDto, CouriersGroupOrders
from app.schemas.responses.couriers import CreateCouriersResponse, GetCouriersResponse, GetCourierMetaInfoResponse
from app.schemas.responses.common import NotFoundResponse, BadRequestResponse
from app.schemas.models.couriers import CourierTypeEnum, CourierDto
...
async def create_courier(
        couriers: list[CourierDto] = Depends(create_courier)
):
...
async def get_couriers_assignments(
        ...
        couriers: list[CouriersGroupOrders] = Depends(get_couriers_assignments),
):
    return OrderAssignResponse(date=date, couriers=couriers)
...
async def get_couriers_in_range(
        ...
        couriers: list[CourierDto] = Depends(get_couriers_in_range),
):
...
async def get_courier_meta_info(
        courier_meta_info: GetCourierMetaInfoResponse = Depends(get_courier_metadata),
        start_date: datetime = Depends(date_to_datetime_start),
        end_date: datetime = Depends(date_to_datetime_end),
        orders: list[OrderDto] = Depends(get_courier_orders_in_time_interval)
):
    if orders:
        ...
        courier_meta_info.rating = round(len(orders) / hours * rating_coefficient)
        for order in orders:
